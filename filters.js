...
function inverted(matrix) {
    return matrixFromFunc(matrix.length, matrix[0].length, function(i, j) {
...
function gaussianMask(M, radius, sigma) {
    ...
    var k = 2 * radius + 1,
        mean = k / 2,
        sum = 0,
        kernel = matrixFromFunc(k, k, function(x, y) {
...
function sobelMask(M) {
    ...
    var Gx = [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]],
        ...
        Csum = matrixFromFunc(Cx.length, Cx[0].length, function(i, j) {
            return clamp(Math.abs(Cx[i][j]) + Math.abs(Cy[i][j]), 0, 255);
        }),
        G = matrixFromFunc(Cx.length, Cx[0].length, function(i, j) {
            if (Cx[i][j] == 0)
...
function nonMaxSuppression(M, G) {
    return matrixFromFunc(M.length, M[0].length, function(i, j) {
...
function estimateThreshold(M) {
    var high_percentage = 0.1, // percentage of pixels that meet the high threshold
        low_percentage = 0.8, // ratio of low threshold to high
        histogram = zeros(1, 256)[0], // length 256 array of zeros
        m = M.length,
        n = M[0].length;
...
function hysteresis(M) {
    ...
    return matrixFromFunc(m, n, function(i, j) {
